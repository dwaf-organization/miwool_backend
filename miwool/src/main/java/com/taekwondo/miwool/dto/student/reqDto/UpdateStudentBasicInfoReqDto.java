...
@NoArgsConstructor
@AllArgsConstructor
public class UpdateStudentBasicInfoReqDto {
    
    // === 필수 (식별자) ===
    private String studentCode;              // 제자코드 (필수, 수정 불가)
    ...
    // === 급수 변경 시 추가 정보 ===
    private Integer taekwondoMonths;         // 태권도 개월수 (급수 변경 시)
    
    // === 재원상태 변경 시 추가 정보 ===
    private LocalDate statusChangeDate;      // 상태 변경일 (null이면 오늘)
